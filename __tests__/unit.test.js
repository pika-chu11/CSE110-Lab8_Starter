...
test("Check valid phone number", () =>{
    ...
    expect(functions.isPhoneNumber(arg3)).toBe(true);
});
...
test("Check valid strong password", () =>{
    ...
    const arg2 = 'hadfadf1';    // contain character other than * _ and numbers
    const arg3 = 'X1231ljl_'; // >= 15 characters

    expect(functions.isStrongPassword(arg)).toBe(false);
    expect(functions.isStrongPassword(arg1)).toBe(false);
    expect(functions.isStrongPassword(arg2)).toBe(true);
    expect(functions.isStrongPassword(arg3)).toBe(true);
});
